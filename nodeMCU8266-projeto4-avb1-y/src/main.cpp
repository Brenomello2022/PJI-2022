...
SMTPSession smtp;
IPAddress ip(192,168,1,171);//Mude o IP
...
message.subject = "chuvarada";
message.addRecipient("MANDELÃO", RECIPIENT_EMAIL);
//Editar a mensagem com HTML
String htmlMsg = "<div style=\"color:#red;\"><h1>CHOVENDO<\/h1><p>- CHEGO O EMAIL <\/p><\/div>";
...
void loop() {
  ...
  if(chuvaDigitalVal == 0){
    ...
    events.send(String("") .c_str(), "end_email", millis());
    Email();
